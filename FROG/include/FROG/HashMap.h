...
template <typename Key, typename T, typename HASH = FROG::hash<Key>> class HashMap {
  public:
  ...
  private:
	struct Entry {
		...
		Entry(const Key &key, Args &&...args) : key(key), value(forward<Args>(args)...) {}
...
template <typename Key, typename T, typename HASH> HashMap<Key, T, HASH>::~HashMap() { clear(); }
...
template <typename Key, typename T, typename HASH> void HashMap<Key, T, HASH>::clear() {
	m_buckets.clear();
	m_size = 0;
}
...
template <typename Key, typename T, typename HASH> bool HashMap<Key, T, HASH>::empty() const {
	return m_size == 0;
}
