...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/08/26 16:02:26 by maroy             #+#    #+#             */
/*   Updated: 2024/08/26 16:02:40 by maroy            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
template <typename T1, typename T2> class Variant {
  public:
	...
	template <typename U> void set(const U &value) { set(move(U(value))); }
...
Variant<T1, T2> &Variant<T1, T2>::operator=(const Variant<T1, T2> &other) {
	clear();
	if (other.is<T1>()) set(other.get<T1>());
	if (other.is<T2>()) set(other.get<T2>());
	return *this;
}
...
Variant<T1, T2> &Variant<T1, T2>::operator=(Variant<T1, T2> &&other) {
	clear();
	if (other.is<T1>()) set(move(other.get<T1>()));
	if (other.is<T2>()) set(move(other.get<T2>()));
	other.clear();
	return *this;
}

template <typename T1, typename T2> template <typename U> bool Variant<T1, T2>::is() const {
	if constexpr (is_same_v<T1, U>) return m_index == 1;
	if constexpr (is_same_v<T2, U>) return m_index == 2;
	return false;
}

template <typename T1, typename T2> template <typename U> void Variant<T1, T2>::set(U &&value) {
...
template <typename T1, typename T2> template <typename U> const U &Variant<T1, T2>::get() const {
	static_assert(is_same_v<T1, U> || is_same_v<T2, U>);
	if constexpr (is_same_v<T1, U>) {
		ASSERT(m_index == 1);
		return *(T1 *) m_storage;
...
template <typename T1, typename T2> template <typename U> U &Variant<T1, T2>::get() {
	static_assert(is_same_v<T1, U> || is_same_v<T2, U>);
	if constexpr (is_same_v<T1, U>) {
		ASSERT(m_index == 1);
		return *(T1 *) m_storage;
...
template <typename T1, typename T2> void Variant<T1, T2>::clear() {
