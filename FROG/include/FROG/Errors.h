...
#define MUST(expr)                                        \
	({                                                    \
		auto e = expr;                                    \
		if (e.is_error()) Kernel::panic("{}", e.error()); \
		e.release_value();                                \
	})
#else
#define MUST(expr)             \
...
#define TRY(expr)                                   \
	({                                              \
		auto e = expr;                              \
		if (e.is_error()) return e.release_error(); \
		e.release_value();                          \
	})
...
template <typename T> class [[nodiscard]] ErrorOr {
  public:
	ErrorOr(const T &value) : m_data(value) {}
	ErrorOr(T &&value) : m_data(move(value)) {}
	ErrorOr(const Error &error) : m_data(error) {}
	ErrorOr(Error &&error) : m_data(move(error)) {}
