...
template <typename T> class Vector {
  ...
  public:
	...
	template <typename... Args> ErrorOr<void> emplace_back(Args &&...);
	template <typename... Args> ErrorOr<void> emplace(size_type, Args &&...);
...
template <typename T> Vector<T>::Vector(size_type size, const T &value) {
...
template <typename T> ErrorOr<void> Vector<T>::push_back(T &&value) {
...
template <typename T> ErrorOr<void> Vector<T>::push_back(const T &value) {
...
template <typename T> bool Vector<T>::contains(const T &other) const {
	for (size_type i = 0; i < m_size; i++)
		if (m_data[i] == other) return true;
	return false;
}
...
template <typename T> ErrorOr<void> Vector<T>::shrink_to_fit() {
...
template <typename T> ErrorOr<void> Vector<T>::ensure_capacity(size_type size) {
