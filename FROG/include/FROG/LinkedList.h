...
template <typename T, bool CONST> class LinkedListIterator;
...
template <typename T> class LinkedList {
  ...
  public:
	...
	template <typename... Args> ErrorOr<void> emplace(iterator, Args &&...);
...
template <typename T> void LinkedList<T>::remove(iterator iter) {
	ASSERT(!empty() && iter);
...
bool LinkedListIterator<T, CONST>::operator==(const LinkedListIterator<T, CONST> &other) const {
	if (m_current != other.m_current) return false;
