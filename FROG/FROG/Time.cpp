...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/21 00:31:15 by maroy             #+#    #+#             */
/*   Updated: 2024/09/21 00:31:21 by maroy            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
FROG::Time from_unix_time(uint64_t unix_time) {
	FROG::Time time{};
	time.second = unix_time % 60;
	unix_time /= 60;
	time.minute = unix_time % 60;
	unix_time /= 60;
	time.hour = unix_time % 24;
	unix_time /= 24;
	...
	for (time.month = 1; time.month < 12; time.month++)
		if (total_days < month_days[time.month] + (is_leap_day || had_leap_day)) break;
