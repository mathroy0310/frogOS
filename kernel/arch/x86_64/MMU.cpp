...
#define CLEANUP_STRUCTURE(s)                   \
	for (uint64_t i = 0; i < 512; i++)         \
		if (s[i] & PRESENT) goto cleanup_done; \
	kfree(s)
...
void MMU::AllocatePage(uintptr_t address) {
	...
	if (!(pt[pte] & PRESENT)) pt[pte] = address | READ_WRITE | PRESENT;
