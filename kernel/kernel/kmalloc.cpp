...
struct kmalloc_info {
	...
	kmalloc_node *from_address(void *addr) {
		for (auto *node = first(); node->end() < end; node = node->after())
			if (node->data() == (uintptr_t) addr) return node;
		return nullptr;
	}
...
template <size_t SIZE> struct kmalloc_fixed_node {
...
void kmalloc_initialize() {
	...
	if ((uintptr_t) &g_kernel_end > s_kmalloc_info.base)
		Kernel::panic("Kmalloc: Kernel end ({}) is over kmalloc base ({})", &g_kernel_end,
		              (void *) s_kmalloc_info.base);
	...
	if (!valid) {
		...
		Kernel::panic("Kmalloc: Could not find {}.{} MB of memory", kmalloc_total_size / MB, kmalloc_total_size % MB);
	}
...
void kmalloc_dump_info() {
	...
	kprintln("kmalloc fixed {} byte: 0x{8H}->0x{8H}", sizeof(kmalloc_fixed_info::node),
	         s_kmalloc_fixed_info.base, s_kmalloc_fixed_info.end);
...
static void *kmalloc_impl(size_t size, size_t align) {
	...
	for (auto *node = info.first(); node->end() <= info.end; node = node->after()) {
		...
		if (auto *next = node->after(); next->end() <= info.end)
			if (!next->used()) node->set_end(next->end());

		if (node->size_no_align() < size) continue;

		ptrdiff_t needed_align = 0;
		if (ptrdiff_t rem = node->data_no_align() % align) needed_align = align - rem;

		if (!node->can_align(needed_align)) continue;
...
void kfree(void *address) {
	...
	if (s_kmalloc_fixed_info.base <= address_uint && address_uint < s_kmalloc_fixed_info.end) {
		...
		if (info.used_list_head == node)
			info.used_list_head = info.used_list_head->prev != kmalloc_fixed_info::node::invalid ?
			                          info.node_at(info.used_list_head->prev) :
			                          nullptr;
		...
		if (auto *next = node->after(); next->end() <= info.end)
			if (!next->used()) node->set_end(node->after()->end());
		node->set_used(false);
