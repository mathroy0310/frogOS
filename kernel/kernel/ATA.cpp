...
static ATADeviceType detect_device_type(uint16_t io_base, uint16_t ctl_base, uint8_t slave_bit) {
	...
	if (lba_mid == 0x00 && lba_high == 0x00) return ATADeviceType::PATA;
	if (lba_mid == 0x14 && lba_high == 0xEB) return ATADeviceType::PATAPI;
	if (lba_mid == 0x3C && lba_high == 0xC3) return ATADeviceType::SATA;
	if (lba_mid == 0x69 && lba_high == 0x96) return ATADeviceType::SATAPI;
...
ATADevice *ATADevice::create(uint16_t io_base, uint16_t ctl_base, uint8_t slave_bit) {
	...
	if (status == 0xFF) return nullptr;

	ATADeviceType type = detect_device_type(io_base, ctl_base, slave_bit);
	if (type == ATADeviceType::UNKNOWN) return nullptr;
...
bool PATADevice::initialize() {
	...
	if (!wait_while_buzy() || !wait_for_transfer()) return false;
	...
	if (!(response[106] & (1 << 15)) && (response[106] & (1 << 14)) && (response[106] & (1 << 12))) {
