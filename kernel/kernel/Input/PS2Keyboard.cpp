...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/21 15:43:49 by maroy             #+#    #+#             */
/*   Updated: 2024/09/21 15:44:15 by maroy            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
FROG::ErrorOr<PS2Keyboard *> PS2Keyboard::create(PS2Controller &controller) {
	PS2Keyboard *keyboard = new PS2Keyboard(controller);
	if (keyboard == nullptr) return FROG::Error::from_errno(ENOMEM);
	FROG::ScopeGuard guard([keyboard] { delete keyboard; });
...
void PS2Keyboard::on_byte(uint8_t byte) {
	...
	case State::Normal: {
		...
		if (byte != 0xE0 && byte != 0xF0) buffer_has_key();
		break;
	}
	}
}

FROG::ErrorOr<void> PS2Keyboard::initialize() {
...
void PS2Keyboard::update() {
	if (m_state == State::WaitingAck) return;
...
void PS2Keyboard::buffer_has_key() {
	...
	if (extended) scancode |= 0x80000000;
	m_byte_index = 0;
...
void PS2Keyboard::update_leds() {
	...
	if (m_modifiers & (uint8_t) Input::KeyEvent::Modifier::NumLock) new_leds |= PS2::Leds::NUM_LOCK;
...
FROG::ErrorOr<void> PS2Keyboard::read(FROG::Span<uint8_t> output) {
	if (output.size() < sizeof(Input::KeyEvent))
		return FROG::Error::from_c_string("Too small buffer for KeyEvent");
