...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/21 15:42:42 by maroy             #+#    #+#             */
/*   Updated: 2024/09/21 15:43:28 by maroy            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static FROG::ErrorOr<void> device_send_byte(uint8_t device, uint8_t byte) {
	if (device == 1) IO::outb(PS2::IOPort::COMMAND, PS2::Command::WRITE_TO_SECOND_PORT);
	...
	return FROG::Error::from_c_string("PS/2 device timeout");
}

static FROG::ErrorOr<uint8_t> device_read_byte() {
	uint64_t timeout = PIT::ms_since_boot() + s_device_timeout_ms;
	while (PIT::ms_since_boot() < timeout)
...
static FROG::ErrorOr<void> device_wait_ack() {
...
FROG::ErrorOr<void> PS2Controller::initialize_impl() {
	...
	if (false) {
		...
		if (!ps2_exists) return FROG::Error::from_c_string("PS2 Controller does not exist");
	}
	...
	if (wait_and_read() != PS2::Response::TEST_CONTROLLER_PASS)
		return FROG::Error::from_c_string("PS/2 Controller self test failed");
	...
	if (config & PS2::Config::CLOCK_SECOND_PORT) {
		...
		controller_send_command(PS2::Command::READ_CONFIG);
		if (!(wait_and_read() & PS2::Config::CLOCK_SECOND_PORT)) valid_ports[1] = true;
		controller_send_command(PS2::Command::DISABLE_SECOND_PORT);
	}
	// Step 8: Perform Interface Tests
	controller_send_command(PS2::Command::TEST_FIRST_PORT);
	if (wait_and_read() != PS2::Response::TEST_FIRST_PORT_PASS) valid_ports[0] = false;
	if (valid_ports[1]) {
		controller_send_command(PS2::Command::TEST_SECOND_PORT);
		if (wait_and_read() != PS2::Response::TEST_SECOND_PORT_PASS) valid_ports[1] = false;
	}
	if (!valid_ports[0] && !valid_ports[1])
		return FROG::Error::from_c_string("No ports available on Controller");

	// Step 9: Enable Devices (and disable scanning)
	for (uint8_t device = 0; device < 2; device++) {
		if (!valid_ports[device]) continue;
		controller_send_command(device == 0 ? PS2::Command::ENABLE_FIRST_PORT : PS2::Command::ENABLE_SECOND_PORT);
		if (set_scanning(device, false).is_error()) valid_ports[device] = false;
	}
	// Step 10: Reset Devices
	for (uint8_t device = 0; device < 2; device++) {
		if (!valid_ports[device]) continue;
		if (reset_device(device).is_error()) valid_ports[device] = false;
		if (set_scanning(device, false).is_error()) valid_ports[device] = false;
	}
	// Step 11: Initialize Device Drivers
	for (uint8_t device = 0; device < 2; device++) {
		if (!valid_ports[device]) continue;
		if (auto res = initialize_device(device); res.is_error()) dprintln("{}", res.error());
	}
	if (m_devices[0]) {
...
FROG::ErrorOr<void> PS2Controller::initialize_device(uint8_t device) {
	...
	if (index == 1 && (bytes[0] == 0x00))
		return FROG::Error::from_c_string("PS/2 mouse not supported");

	// MF2 Keyboard
	if (index == 2 && (bytes[0] == 0xAB && bytes[1] == 0x83))
		m_devices[device] = TRY(PS2Keyboard::create(*this));
	if (m_devices[device]) return {};
	return FROG::Error::from_format("Unhandled PS/2 device {}{} ({} bytes)", bytes[0], bytes[1], index);
}
...
FROG::ErrorOr<void> PS2Controller::reset_device(uint8_t device) {
	TRY(device_send_byte(device, PS2::DeviceCommand::RESET));
	TRY(device_wait_ack());
	if (TRY(device_read_byte()) != PS2::Response::SELF_TEST_PASS)
		return FROG::Error::from_c_string("Device reset failed");
	return {};
}

FROG::ErrorOr<void> PS2Controller::set_scanning(uint8_t device, bool enabled) {
