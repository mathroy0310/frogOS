...
void Scheduler::initialize() {
	...
	ASSERT(s_instance);
	MUST(s_instance->add_thread(FROG::Function<void()>([] {
		for (;;)
			asm volatile("hlt");
	})));
}
...
void Scheduler::reschedule() {
	...
	if (m_last_reschedule + ms_between_switch > current_time) return;
...
void Scheduler::switch_thread() {
	...
	// if (m_threads.size() == 2 && current.id() != 0 && current.state() ==
	// Thread::State::Running) 	return;
