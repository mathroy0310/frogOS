...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/08/19 00:13:17 by maroy             #+#    #+#             */
/*   Updated: 2024/08/19 00:13:24 by maroy            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool DiskDevice::initialize_partitions() {
	...
	if (!read(1, 1, lba1.data())) return false;
	...
	for (const Partition &partition : m_partitions) {
		uint8_t zero[16] = {};
		if (memcmp(partition.type_guid, zero, 16) == 0) continue;

		dprintln("partition:");
		dprintln("    type  {16H}{16H}", *(uint64_t *) partition.type_guid,
		         *(uint64_t *) (partition.type_guid + 1));
		dprintln("    guid  {16H}{16H}", *(uint64_t *) partition.guid,
		         *(uint64_t *) (partition.guid + 1));
